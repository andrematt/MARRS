...
# Load data from CSV
df_train = pd.read_csv("./main/data/fold_5_train.csv", encoding="latin-1", sep=";", index_col=False)
df_test = pd.read_csv("./main/data/fold_5_test.csv", encoding="latin-1", sep=";", index_col=False)
...
combined_contexts = pd.concat([df_train["context"], df_test["context"]], axis=0).astype("category")
...
df_train["context_encoded"] = combined_contexts[:len(df_train)].cat.codes
df_test["context_encoded"] = combined_contexts[len(df_train):].cat.codes

# Convert encoded columns to numpy arrays
users = df_train["user_encoded"].to_numpy()
items = df_train["item_encoded"].to_numpy()
contexts = df_train["context_encoded"].to_numpy()

val_users = df_test["user_encoded"].to_numpy()
val_items = df_test["item_encoded"].to_numpy()
val_contexts = df_test["context_encoded"].to_numpy()
...
def data_generator(users, items, contexts, num_items, batch_size, m, alpha):
    while True:
        # Positive samples
        ...
        context_batch_neg = np.repeat(context_batch, m)
        ...
        combined_context_batch = np.concatenate([context_batch, context_batch_neg])
...
val_generator = data_generator(val_users, val_items, val_contexts, num_items, batch_size, m, alpha)
...
history = model.fit(
    data_generator(users, items, contexts, num_items, batch_size, m, alpha),
...
history_df.to_csv("./main/data/c-ncf_split_5.csv", index=False)
