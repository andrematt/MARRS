...
X_train = np.hstack([user_train, item_train, context_train])
y_train = train_data['label'].to_numpy()
print("HERE 9")
X_test = np.hstack([user_test, item_test, context_test])
y_test = test_data['label'].to_numpy()

print("Starting to train!!")

# Train a Logistic Regression model
model = LogisticRegression(max_iter=1000, solver='liblinear', random_state=42,verbose=3,n_jobs=-1)
...
results_df.to_csv(f'with_ids_logistic_fold_{i}.csv', index=False)

print(f'with_ids_logistic_fold_{i}.csv')
