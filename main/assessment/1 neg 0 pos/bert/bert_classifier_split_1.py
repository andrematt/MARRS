...
path = "your_path/data/1 neg 0 pos"

train=pd.read_csv(path + "/fold_1_train.csv", encoding="latin-1", sep=",", index_col=False)
train.head()

test=pd.read_csv(path + "/fold_1_test.csv", encoding="latin-1", sep=",", index_col=False)
test.head()
...
history = model.fit(train_element, train_label, validation_data=[test_element, test_label], epochs=40, batch_size = 8, callbacks=[early_stopping]) # i train_element_no_punct sono stati creati e poi non usati

# Convert the history to a pandas DataFrame
history_df = pd.DataFrame(history.history)

# Save the history to a CSV file
history_df.to_csv('bert_split_1.csv', index=False)
