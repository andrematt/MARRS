...
df_train=pd.read_csv(path + "/fold_3_train.csv", encoding="latin-1", sep=",", index_col=False)
df_train.head()

df_test=pd.read_csv(path + "/fold_3_test.csv", encoding="latin-1", index_col=False)
df_test.head()
...
model.compile(loss=keras.losses.BinaryCrossentropy(from_logits=True), optimizer=tf.keras.optimizers.Adam(learning_rate=0.01), metrics=['accuracy', 'AUC', 'Precision', 'Recall'])
...
history_df.to_csv('simple_ncf_split_3.csv', index=False)
